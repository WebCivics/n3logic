...
function boolLiteral(val: boolean): N3Term {
  return { type: 'Literal', value: val ? 'true' : 'false', datatype: XSD_BOOLEAN };
}

function numLiteral(val: number): N3Term {
  if (Number.isInteger(val)) {
    return { type: 'Literal', value: String(val), datatype: XSD_INTEGER };
  } else {
    return { type: 'Literal', value: String(val), datatype: XSD_DECIMAL };
  }
}
...
export const MathBuiltins: N3Builtin[] = [
  ...
  apply: (x: N3Term, y: N3Term): N3Term => boolLiteral(Number(getValue(x)) > Number(getValue(y)))
  },
  {
  ...
  apply: (x: N3Term, y: N3Term): N3Term => boolLiteral(getValue(x) === getValue(y))
  },
  {
  ...
  apply: (x: N3Term, y: N3Term): N3Term => numLiteral(Number(getValue(x)) + Number(getValue(y)))
  },
  {
  ...
  },
  {
    ...
    apply: (x: N3Term, y: N3Term): N3Term => {
      const divisor = Number(getValue(y));
      if (divisor === 0) {
        return { type: 'Literal', value: 'NaN', datatype: XSD_DECIMAL };
      }
      return numLiteral(Number(getValue(x)) / divisor);
    }
  },
  {
  ...
  apply: (x: N3Term): N3Term => numLiteral(Math.abs(Number(getValue(x))))
  },
  {
    uri: 'http://www.w3.org/2000/10/swap/math#power',
    arity: 2,
    description: 'math:power(x, y) returns x^y',
  apply: (x: N3Term, y: N3Term): N3Term => numLiteral(Math.pow(Number(getValue(x)), Number(getValue(y))))
  },
  {
  ...
  apply: (x: N3Term): N3Term => boolLiteral(isFinite(Number(getValue(x))))
  },
  {
    uri: 'http://www.w3.org/2000/10/swap/math#isNaN',
    arity: 1,
    description: 'math:isNaN(x) is true if x is NaN',
  apply: (x: N3Term): N3Term => boolLiteral(isNaN(Number(getValue(x))))
  }
];
